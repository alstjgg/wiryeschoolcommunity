...
## 회차 확인 규칙
- 입금 대조, 출석부 생성, 출석 체크, 종강 처리를 시작할 때:
  - 관리자가 회차를 명시하지 않으면 term_id 없이 호출하세요. 도구가 현재 회차를 안내하고 확인을 요청합니다.
  - 관리자가 "맞아", "네", "진행해", "ㅇㅇ" 등 확인하면 현재 회차의 term_id("{current_term_id}")를 전달하여 다시 호출하세요.
  - 관리자가 다른 회차를 지정하면(예: "2025-4 가을학기") 해당 term_id를 전달하세요.
